import React from "react";
import Image from "next/image";
import fontTitle from "@/lib/font";

const ContactMapSection = () => (
  <section className="w-full bg-white overflow-x-hidden">
    {/* Heading */}
    <div className="flex flex-col my-8 md:my-16 lg:my-12 gap-4 md:gap-6 container mx-auto text-center px-4 md:px-0" data-aos="fade-up">
      <h3 className={`text-3xl md:text-[58px] font-light tracking-wide ${fontTitle.className}`}>
        Venir à Clé Blanche Souissi
      </h3>

      <p className="text-sm md:text-[20px] leading-[1.2] md:leading-[1.25] max-w-4xl mx-auto">
        Clé Blanche est très bien desservie et facilement accessible depuis la
        rocade de Rabat. Pour accéder à Google map, cliquez simplement sur la
        carte ci-dessous. <br /> À très bientôt!
      </p>
    </div>

    {/* Full-width image, no cropping */}
    <div className="w-screen max-w-none">
      <Image
        src="/contact/IMAGE PLAN CLE BLANCHE.png"
        alt="Plan de situation Clé Blanche"
        width={1600}            // source image natural width
        height={900}            // source image natural height
        sizes="100vw"
        priority
        className="w-screen h-auto object-contain"
      />
    </div>
  </section>

  // <section className="py-16 bg-gradient-to-br from-main/10 via-white to-main/5">
  //   <div className="container mx-auto px-5">
  //     {/* Two-column layout for desktop, stacked for mobile */}
  //     <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start max-w-6xl mx-auto">
  //       {/* Situation Plan */}
  //       <div className="w-full" data-aos="fade-up">
  //         <h3
  //           className={`text-2xl lg:text-3xl font-semibold text-main mb-4 text-center ${fontTitle.className}`}
  //         >
  //           Plan de situation
  //         </h3>
  //         <Link
  //           href={"https://maps.app.goo.gl/GhEThjNwgyUoGhpw8"}
  //           target="_blank"
  //           className="block w-full rounded-xl overflow-hidden shadow-xl border-2 border-main/20 bg-white hover:shadow-2xl transition-shadow duration-300"
  //         >
  //           <Image
  //             src="/contact/IMAGE PLAN CLE BLANCHE.png"
  //             alt="Plan Clé Blanche"
  //             width={900}
  //             height={400}
  //             className="object-contain w-full transition-transform duration-300 hover:scale-105"
  //             priority
  //           />
  //         </Link>
  //       </div>

  //       {/* Google Maps Embed - Same size as first map */}
  //       <div className="w-full" data-aos="fade-up">
  //         <h3
  //           className={`text-2xl lg:text-3xl font-semibold text-main mb-4 text-center ${fontTitle.className}`}
  //         >
  //           Carte interactive
  //         </h3>
  //         <div className="w-full h-[300px] rounded-xl overflow-hidden shadow-xl border-2 border-main/20 bg-white">
  //           <iframe
  //             src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d6612.5!2d-6.8416812!3d33.9526197!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0xda713372495f19d%3A0xd075b9047f518df9!2eCl%C3%A9%20Blanche%20Souissi!5e0!3m2!1sfr!2sma!4v1640000000000!5m2!1sfr!2sma&markers=color:red%7Clabel:C%7C33.9526197,-6.8416812"
  //             width="100%"
  //             height="100%"
  //             style={{ border: 0 }}
  //             allowFullScreen
  //             loading="lazy"
  //             referrerPolicy="no-referrer-when-downgrade"
  //             title="Localisation Clé Blanche"
  //             className="w-full h-full"
  //           />
  //         </div>
  //       </div>
  //     </div>
  //   </div>
  // </section>
);

export default ContactMapSection;
